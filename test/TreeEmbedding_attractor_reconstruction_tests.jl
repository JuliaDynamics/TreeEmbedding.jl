...
@time begin
@testset "TreeEmbedding single rollout on univariate data" begin
...
@testset "TreeEmbedding single rollout on multivariate data" begin

    Random.seed!(1234)
    tws = 2:4:delays[end]
    optmodel = TreeEmbedding.MCDTSOptimGoal(TreeEmbedding.L_statistic(-0.05,3,tws), TreeEmbedding.Continuity_function())
    tree = TreeEmbedding.mc_delay(data, optmodel, w, delays, runs2)
    best_node = TreeEmbedding.best_embedding(tree)
    
    Random.seed!(1234)
    optmodel2 = TreeEmbedding.MCDTSOptimGoal(TreeEmbedding.FNN_statistic(0.05), TreeEmbedding.Continuity_function())
    tree2 = TreeEmbedding.mc_delay(data, optmodel2, w, delays, runs2)
    best_node2 = TreeEmbedding.best_embedding(tree2)
    L_YY = TreeEmbedding.compute_delta_L(data, best_node2.τs, best_node2.ts, delays[end];  w = w, tws = 2:4:delays[end])
    ...
    tree3 = TreeEmbedding.mc_delay(data, optmodel2, w, delays, runs2)
    best_node3 = TreeEmbedding.best_embedding(tree3)
