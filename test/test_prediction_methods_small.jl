...
begin
    ...
    optmodel = TreeEmbedding.MCDTSOptimGoal(TreeEmbedding.PredictionError(PredType,0,samplesize), TreeEmbedding.Continuity_function(13,samplesize))
end
...
for T = 1:T_steps
    println(T)
    # iterated one step
    predicted_cao, _ = TreeEmbedding.local_zeroth_prediction(prediction_cao, K1; theiler = w1)
    push!(prediction_cao, predicted_cao)
    predicted_kennel, _ = TreeEmbedding.local_zeroth_prediction(prediction_kennel, K1; theiler = w1)
    push!(prediction_kennel, predicted_kennel)
    predicted_hegger, _ = TreeEmbedding.local_zeroth_prediction(prediction_hegger, K1; theiler = w1)
    push!(prediction_hegger, predicted_hegger)
    predicted_mcdts, _ = TreeEmbedding.local_zeroth_prediction(prediction_mcdts, K1; theiler = w1)
    push!(prediction_mcdts, predicted_mcdts)
    # linear
    # predicted_cao, _ = TreeEmbedding.local_linear_prediction(prediction_cao, K1; theiler = w1)
    # push!(prediction_cao, predicted_cao)
    # predicted_kennel, _ = TreeEmbedding.local_linear_prediction(prediction_kennel, K1; theiler = w1)
    # push!(prediction_kennel, predicted_kennel)
    # predicted_hegger, _ = TreeEmbedding.local_linear_prediction(prediction_hegger, K1; theiler = w1)
    # push!(prediction_hegger, predicted_hegger)
    # predicted_mcdts, _ = TreeEmbedding.local_linear_prediction(prediction_mcdts, K1; theiler = w1)
    # push!(prediction_mcdts, predicted_mcdts)
end
...
begin
    ...
    subplot(4,1,4)
    plot(t2[8000:end], tr[8000:end-offset,1], ".-", label="true data")
    plot(t2[end-T_steps:end], prediction_mcdts[end-T_steps:end,1], ".-", label="TreeEmbedding")
    title("x-component (iterated one-step)")
    xlim(-5, 12)
    ylim(-20,20)
    xlabel("Lyapunov time units")
